  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="w-full max-w-6xl max-h-[92vh] overflow-hidden rounded-xl border border-border bg-card">
        ...
        <div className="p-4 pt-0 overflow-auto max-h-[68vh] grid md:grid-cols-2 xl:grid-cols-3 gap-3">
          {list.map(({ raw, view }) => (
            <button key={raw.id} onClick={() => onPick(raw.id)} className="text-left rounded-lg border border-border bg-background/40 p-3 hover:border-gold/50 transition">
              ...
              <div className="mt-2 text-xs text-gold">Equiper</div>
            </button>
          ))}
        </div>
      </div>
